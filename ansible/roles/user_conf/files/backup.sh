...
do
    ...
    archive_name="$(hostname)-$name-$(date +%Y_%m_%d).tar.gz"
    log_file_name="$(hostname)-$name-$(date +%Y_%m_%d).log"


    echo "Running '$name' cleanup: deleting backup older than $retention_days days from s3://$dest_bucket " | tee $log_file_name
    for s3_file in $(aws s3 ls $dest_bucket | grep "$(hostname)-$name" | awk '{ print $4 }')
    do
